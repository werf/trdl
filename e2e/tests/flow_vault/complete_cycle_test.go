	...
	DescribeTable("should perform all steps",
		func(testOpts testOptions) {
			...
			By(fmt.Sprintf("[server] Releasing tag %q ...", testOpts.tag1))
			{
				...
				By(fmt.Sprintf("[server] Releasing tag %q", testOpts.tag1))
				serverRelease(testOpts.projectName, testOpts.tag1, false)
			}
			...
			By(fmt.Sprintf("[server] Releasing tag %q ...", testOpts.tag2))
			{
				...
				serverRelease(testOpts.projectName, currentTag, false)
			...
			By("Registering another plugin")
			{
				newProject := "test2"
				serverInitProject(SuiteData.TestDir, newProject)
				serverConfigureProject(SuiteData.TestDir, serverConfigureOptions{
				...
				serverReadProjectConfig(SuiteData.TestDir, newProject)
				currentTag := "v1.0.4"
				gitTag(SuiteData.TestDir, currentTag, testOpts.pgpKeys["developer"])
				serverRelease(newProject, currentTag, true)
			}
