...
Вы можете использовать секреты во время сборки, предварительно добавив их в хранилище секретов с помощью метода [`configure/secrets`]({{ "/reference/vault_plugin/configure/secrets.html" | true_relative_url }}). Секреты мотируются по пути `/run/secrets/<id>`, где `id` - это идентификатор секрета использованного при добавлении секрета в хранилище. Пример использования:
