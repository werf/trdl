...
#### Сборка без CLI docker

Оба buildx-драйвера выше запускают CLI `docker`, поэтому требуют наличия бинарника рядом с плагином. Если плагин работает в окружении без бинарника `docker` — например, встроен в другой процесс, поставляемый в distroless-образе, — собрать всё равно можно двумя способами:

* `buildkitd_driver` — плагин сам поднимает временный `buildkitd`, по одному на сборку, и удаляет его после неё;
* `buildkitd_address` — плагин подключается к `buildkitd`, который запускает кто-то другой, и ничего не создаёт.

Оба обращаются к BuildKit через Go-клиент и не запускают внешних бинарников. Задать можно только один из них, и ни один нельзя сочетать с настройками buildx выше.
...
| Опция | Значение |
|---|---|
| `namespace` | namespace для сборщика; по умолчанию — namespace собственного ServiceAccount плагина либо текущего контекста kubeconfig |
...
| `deadline` | предельное время жизни пода сборщика, например `2h`; по умолчанию не задано |
...
* namespace должен существовать, и — как и для buildx-драйвера `kubernetes` — BuildKit требует ослабленного профиля seccomp/AppArmor, поэтому namespace должен быть помечен как `privileged` либо исключён из PodSecurity-admission. В отличие от buildx-пути, отказ приходит прямо из вызова `create`, а не в виде таймаута готовности;
* под удаляется по окончании сборки, в том числе при её падении и отмене, а также если сборщик так и не стал готов. Он не удаляется, если сам процесс плагина умер целиком, — на этот случай задайте `deadline`, ограничив время жизни брошенного сборщика;
