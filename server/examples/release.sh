...
GIT_TAG="${GIT_TAG:?ERROR: \$GIT_TAG variable required!}"
...
out=$(curl -sS -q -X PUT -H "X-Vault-Token: $VAULT_TOKEN" -H "X-Vault-Request: true" -d "{\"git_tag\":\"${GIT_TAG}\"}" $VAULT_ADDR/v1/$PROJECT_NAME/release || (echo "Error: unable to start release operation">&2 && exit 1))
echo "${out}" | jq -e .errors > /dev/null && echo -e "Error: release operation request failed:\n${out}">&2 && exit 1
